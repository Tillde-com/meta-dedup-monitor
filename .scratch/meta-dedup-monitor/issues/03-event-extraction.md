...
- [ ] CAPI batch `{data:[{event_name:"Purchase",event_id:"e1",...},{event_name:"Lead",event_id:"e2"}]}` → 2 rows, correct names/ids.
- [ ] Single object with alias keys (`eventName`, `eventID`) → 1 row with normalized fields.
- [ ] JSON array of 3 events → 3 rows.
- [ ] Form-urlencoded body `ev=PageView&eid=e9&_fbp=fb.1.x` → 1 row: `event_name='PageView'`, `event_id='e9'`, `fbp='fb.1.x'`.
- [ ] Browser GET with query `?event_name=AddToCart&event_id=e5&fbc=fb.1.y&d=<json>` → 1 row, `fbc` captured.
- [ ] Nested fields (e.g. `{event:{eventId:"deep1"}}`-style nesting as in reference fixtures) found by deep search.
- [ ] Unparseable body (`content-type: text/plain`, body `garbage`) → exactly 1 fallback row, null `event_id`, raw preserved.
- [ ] `external_id` and `event_time` extracted when present (CAPI-style payload).
- [ ] All rows carry the `request_id` of their `requests` row and `source` of the channel.
